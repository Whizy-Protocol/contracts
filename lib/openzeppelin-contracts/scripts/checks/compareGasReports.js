  ...
  static compare(update, ref, opts = { hideEqual: true, strictTesting: false }) {
    ...
    const methods = Object.keys(updateInfo.methods)
      ...
      .map(key => ({
        ...
        min: variation(...[updateInfo, refInfo].map(x => Math.min(...x.methods[key].gasData)), WHIZY_TX_COST),
        max: variation(...[updateInfo, refInfo].map(x => Math.max(...x.methods[key].gasData)), WHIZY_TX_COST),
        avg: variation(...[updateInfo, refInfo].map(x => Math.round(average(...x.methods[key].gasData))), WHIZY_TX_COST),
      }))
      .sort((a, b) => `${a.contract}:${a.method}`.localeCompare(`${b.contract}:${b.method}`));
